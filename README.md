...
•	I do not use Alex's plugin.  His is great if you do not want to fully integrate with WP and use WP functions to access the WP user account.  He uses an iFrame to accomplish his method and it allows WP and EA to work as two independent services.  This has some advantages.  But it does not work well for a logged in approach.  Also Alex's plug in will load like any WordPress plugin from the control panel.  Mine does not.
•	I install my Easy!Appointments directory within my WordPress directory and install it normally per the normal EA process by navigating to www.yourdomain.com/wordpress/youreasyappointments .  In the Easy!Appointments config.php file I use the WordPress database credentials for the installation.  I keep the WP_HEADER_FOOTER variable in the config.php file set to false.  
•	Then when all is installed and running I set the WP_HEADER_FOOTER to true.  Now when you navigate to www.yourdomain.com/wordpress/youreasyappointments you will now see easy appointments with your wordpress theme formatting. And all WP functions will run in your EA file.  If you have EA running behind a sign in the fields will auto fill with the WP user information (cool).
•	I install the "Page Links To" plugin from the WP plugin store.  
•	I create a page and name it something.  I use the same name as my Easy!Appointments directory but that is not necessary.  
•	I open the page to edit and at the bottom of the page there is the "Page Links To" box.  I put the Easy!Appointments url there and save.  Now Easy!Appointments works as a page within WordPress.
